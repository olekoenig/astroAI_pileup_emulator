...
def train_model(model, train_loader, val_loader, criterion, optimizer,
    ...
    # best_val_loss = float('inf')  # Best validation loss (to stop early)
    # epochs_without_improvement = 0  # Counter for early stopping
    train_losses = []
    val_losses = []

    for epoch in range(num_epochs):
        avg_train_loss = training_loop(model, train_loader, criterion, optimizer)
...
#            weightfile = config.DATA_NEURAL_NETWORK + "model_weights.pth"
...
def main():
    ...
    train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=128)
