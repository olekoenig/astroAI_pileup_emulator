...
ds9 repro/acisf${obsid_padded}_evt1a.fits -scale log -region load ${srcfile} -region load ${bkgfile} -saveimage inspect.jpeg -quit

## Extract 0th order undispersed (potentially piled-up) spectrum
echo "Running specextract for ObsID ${obsid}"
punlearn specextract
specextract "repro/acisf${obsid_padded}_repro_evt2.fits[sky=region(${srcfile})][tg_m=0,Null]" zeroth_order bkgfile="repro/acisf${obsid_padded}_repro_evt2.fits[sky=region(${bkgfile})]"
